...
from baselines.diabetic_retinopathy_detection.utils import (
  fast_load_dataset_to_model_results,
)
...
def get_name_to_severity_mapping(files, name_col="image", level_col="level"):
  ...
  for file_path in files:
    with tio.GFile(file_path) as f:
      ...
      to_update = {
        df[name_col].iloc[i]: df[level_col].iloc[i] for i in range(df.shape[0])
      }
...
def get_name_to_label_mapping() -> Dict[str, int]:
  ...
  valid_and_test_labels = (
    "gs://ub-data/retinopathy/downloads/manual/validationAndTestLabels.csv"
  )
  name_to_severity = get_name_to_severity_mapping(
    [train_labels, valid_and_test_labels], name_col="image", level_col="level"
  )
  aptos_labels = "gs://ub-data/aptos/metadata.csv"
  aptos_mapping = get_name_to_severity_mapping(
    [aptos_labels], name_col="id_code", level_col="diagnosis"
  )
...
def get_model_joint_results(
  dataset_to_model_results, model, datasets=["in_domain_test", "ood_test"]
):
  ...
  for field in fields:
    ...
    for dataset in datasets[1:]:
      for i in range(len(l)):
        l[i] = np.concatenate(
          [l[i], dataset_to_model_results[dataset][model][field][i]]
        )
    result[field] = l
  return result
...
def plot_result_grid_sep_accuracy(
  result,
  name_to_severity,
  field="y_pred_entropy",
  i=0,
  bins=50,
  normalized=False,
  axes=None,
  ...
  **kwargs,
  ...
  def _plot(ax, j):
    ...
    ax.set_title(f"{DISPLAY_MODEL_NAME[model]}", fontsize=fontsize)
    if percentage:
      ax.yaxis.set_major_formatter(PercentFormatter(1))

  get_kwargs = lambda j: {"j": j}
  axes = grid_plot_wrapper(
    _plot, n_plots=len(partitions), ncols=ncols, get_kwargs=get_kwargs, axes=axes
  )
  return axes
...
def show_several_models(
  ...
  **kwargs,
  ...
  if percentage and normalized:
    raise ValueError(
      "At most one should be set to True for percentage and normalized"
    )
  results = {
    m: get_model_joint_results(dataset_to_model_results, m, datasets)
    for m in models
  }

  fig, axes = plt.subplots(
    nrows=5, ncols=len(models), sharex=True, sharey=True, figsize=figsize
  )
  for i, m in enumerate(models):
    model_axes = axes[:, i: i + 1] if axes.ndim == 2 else axes[:, None]
    result = results[m]
    plot_result_grid_sep_accuracy(
      result,
      name_to_severity,
      i=i,
